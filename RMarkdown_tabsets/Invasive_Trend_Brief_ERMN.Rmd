...
Eastern Rivers and Mountains Network {.tabset .tabset-fade}
------------------------------------
### Background 
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="ALPO") %>% droplevels()
...
tot_results<-totinv_to_sentences("ALPO")
guild_results<-guild_to_sentences("ALPO")
spp_results<-spp_to_sentences("ALPO")
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in ALPO. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "ALPO", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","ALPO", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "ALPO", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="BLUE") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("BLUE")
guild_results<-guild_to_sentences("BLUE")
spp_results<-spp_to_sentences("BLUE")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in BLUE. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "BLUE", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","BLUE", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "BLUE", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="DEWA") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("DEWA")
guild_results<-guild_to_sentences("DEWA")
spp_results<-spp_to_sentences("DEWA")
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in DEWA. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "DEWA", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","DEWA", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "DEWA", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="FONE") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("FONE")
guild_results<-guild_to_sentences("FONE")
spp_results<-spp_to_sentences("FONE")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in FONE. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "FONE", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","FONE", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "FONE", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="FRHI") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("FRHI")
guild_results<-guild_to_sentences("FRHI")
spp_results<-spp_to_sentences("FRHI")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in FRHI. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "FRHI", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","FRHI", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "FRHI", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="GARI") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("GARI")
guild_results<-guild_to_sentences("GARI")
spp_results<-spp_to_sentences("GARI")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in GARI. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "GARI", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","GARI", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "GARI", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="JOFL") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("JOFL")
guild_results<-guild_to_sentences("JOFL")
spp_results<-spp_to_sentences("JOFL")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in JOFL. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "JOFL", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","JOFL", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "JOFL", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="NERI") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("NERI")
guild_results<-guild_to_sentences("NERI")
spp_results<-spp_to_sentences("NERI")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in NERI. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "NERI", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","NERI", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "NERI", 
                              c(0,20,40,60,80,100))
