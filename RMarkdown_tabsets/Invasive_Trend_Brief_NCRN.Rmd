...
National Capital Region Network {.tabset .tabset-fade}
------------------------------------
### Background 
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="ANTI") %>% droplevels()
...
tot_results<-totinv_to_sentences("ANTI")
guild_results<-guild_to_sentences("ANTI")
spp_results<-spp_to_sentences("ANTI")
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in ANTI. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "ANTI", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","ANTI", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "ANTI", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="CATO") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("CATO")
guild_results<-guild_to_sentences("CATO")
spp_results<-spp_to_sentences("CATO")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in CATO. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "CATO", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","CATO", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "CATO", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="CHOH") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("CHOH")
guild_results<-guild_to_sentences("CHOH")
spp_results<-spp_to_sentences("CHOH")
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in CHOH. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "CHOH", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","CHOH", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "CHOH", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="GWMP") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("GWMP")
guild_results<-guild_to_sentences("GWMP")
spp_results<-spp_to_sentences("GWMP")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in GWMP. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "GWMP", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","GWMP", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "GWMP", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="HAFE") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("HAFE")
guild_results<-guild_to_sentences("HAFE")
spp_results<-spp_to_sentences("HAFE")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in HAFE. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "HAFE", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","HAFE", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "HAFE", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="MANA") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("MANA")
guild_results<-guild_to_sentences("MANA")
spp_results<-spp_to_sentences("MANA")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in MANA. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "MANA", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","MANA", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "MANA", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="MONO") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("MONO")
guild_results<-guild_to_sentences("MONO")
spp_results<-spp_to_sentences("MONO")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in MONO. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "MONO", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","MONO", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "MONO", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="NACE") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("NACE")
guild_results<-guild_to_sentences("NACE")
spp_results<-spp_to_sentences("NACE")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in NACE. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "NACE", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","NACE", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "NACE", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="PRWI") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("PRWI")
guild_results<-guild_to_sentences("PRWI")
spp_results<-spp_to_sentences("PRWI")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in PRWI. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "PRWI", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","PRWI", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "PRWI", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="ROCR") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("ROCR")
guild_results<-guild_to_sentences("ROCR")
spp_results<-spp_to_sentences("ROCR")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))
```
In the most recent cycle in `r site1$park` (`r site1$park_code`; 2015-2018), at least one invasive species was detected in `r check_too_small(parkinv$plot.pfreq[1])`\% of plots, and on average, `r check_too_small(parkinv$quad.pfreq[1])`\% of quadrats had at least one invasive species and average total invasive cover was `r check_too_small(parkinv$avg.cover[1])`\% (Table 1). The `r str_to_lower(parkguild$guild)` guild was the most abundant guild in `r site1$park_code`, found in `r check_too_small(parkguild$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkguild$quad.pfreq[1])`\% quadrat frequency, and `r check_too_small(parkguild$avg.cover[1])`\% quadrat cover (Figure 1). The most abundant invasive species in `r site1$park_code` was `r paste0(parkspp$Common[1])` (<i>`r paste0(parkspp$sppgroup[1])`</i>), occurring in `r check_too_small(parkspp$plot.pfreq[1])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[1])`\% cover. `r str_to_sentence(paste0(parkspp$Common[2]))` (<i>`r paste0(parkspp$sppgroup[2])`</i>) was the second most abundant invasive species, occurring in `r check_too_small(parkspp$plot.pfreq[2])`\% of plots, and averaging `r check_too_small(parkspp$avg.cov[2])` \% cover. The next three most abundant invasive species in `r site1$park_code` were `r paste0(parkspp$Common[3])` (<i>`r paste0(parkspp$sppgroup[3])`</i>), `r paste0(parkspp$Common[4])` (<i>`r paste0(parkspp$sppgroup[4])`</i>), and `r paste0(parkspp$Common[5])` (<i>`r paste0(parkspp$sppgroup[5])`</i>).
...
```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in ROCR. Solid symbols and lines indicate significant trends. Error bars are 95% confidence intervals.', fig.align='center'}
plot_pf<-plotfreq_park_by_guild(pfreq_guild_comb, "Plot % Frequency", "ROCR", c(0,20,40,60,80,100))
plot_qf<-quaddf_park_by_guild(QF_guild_resp, "Quadrat % Frequency","ROCR", c(0,20,40,60,80,100))
plot_ac<-quaddf_park_by_guild(AC_guild_resp, "Average % Cover", "ROCR", 
                              c(0,20,40,60,80,100))
...
```{r, echo=F, include=F}
library(tidyverse)
site1<-siteinfo %>% filter(park_code=="WOTR") %>% droplevels()
park_code<-as.character(site1$park_code)
...
tot_results<-totinv_to_sentences("WOTR")
guild_results<-guild_to_sentences("WOTR")
spp_results<-spp_to_sentences("WOTR")
no_trends <- length(unique(tot_results))+ length(unique(guild_results))+
  length(unique(spp_results))

# Summarize data for guild figures. df comes from trend_summary_functions.R
WOTR_guild<-df %>% filter(park=="WOTR") %>% 
...
WOTR_qfreq_by_guild<-function(df, title){
...
} # plots results for WOTR

WOTR_avgcov_by_guild<-function(df, title){
...
WOTR_plotfreq_by_guild<-function(df, title, ylabels){
...
<h3>Trends</h3>
Sample size was insufficient to model trends in WOTR.

```{r echo=F,warning=F, message=F, results='asis',fig.height='50px',fig.asp=0.4, fig.cap='Figure 1. Trends by guild and cycle in WOTR. Sample size was insufficient in WOTR to analyze for trends. Circles represent the mean value for each metric by cycle.', fig.align='center'}
plot_pf<-WOTR_plotfreq_by_guild(WOTR_guild, "Plot % Frequency", c(0,20,40,60,80,100))
plot_qf<-WOTR_qfreq_by_guild(WOTR_guild, "Quadrat % Frequency")
plot_ac<-WOTR_avgcov_by_guild(WOTR_guild, "Average % Cover")
